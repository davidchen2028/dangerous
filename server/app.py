...
AI_LEVELS = (
    ...
    "Level 6.1：零食室。"
    "Level 7：无尽之海，生存难度 4；破木屋停在海面上，临海缺口跳进水会沉没，约 10 秒后去 Level 8。会淹的是这里，不是 21。"
    "Level 8：岩洞系统，生存难度 4，第九大道通向 Level 9。"
    ...
    "Level 21：编号门。中央花园加十字走廊，生存难度 4。走廊有死亡飞蛾和肢团；写着 204 的门通往 Level 81。不是水层。"
    "Level 81：赋闲结局，生存难度 0。办公室喧嚣褪成旧毛毯和柠檬清香；看画、听雨，坐到中间的毯子上休息，直到月亮报晓。雨窗玻璃上若凝着虚数 i，摸到会去 Level √2。谷仓办公门和 204 号门可进入。没有实体。"
    "Level √2：无理数，生存难度 Σ。青色纤维网与黄色节点，非欧几里得数学幻梦；从 Level 81 找到虚数进入。纤维可穿过。任意一扇门回到未经计算之物（Level 0）。时空裂缝去 Level C-1623。"
    "Level C-1623：二十里面有过量的三十，生存难度 Σ。硬沥青路，两端虚空换 PRS−40～+40 区段。路边门多数出不去；已做层级的门能走。向上突破去蓝色通道。从 √2 的裂缝进入。没有远区段捷径。"
